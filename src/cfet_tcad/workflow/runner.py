...
from ..geometry.params import GATE_CONTACTS
from ..io import (
    plot_idvd,
    plot_idvg,
    write_iv_csv,
...
class Runner:
    ...
    def build_mesh(self) -> Path:
        ...
        builder = Nanosheet2DBuilder(self.cfg.device, self.cfg.mesh)
        self.layout = builder.build(msh)
        return msh
    ...
    def run_idvg(self) -> dict:
        ...
        for vd in sim.vd:
            self._ramp(list(GATE_CONTACTS), sim.vg_start, sim.vg_step)
            self._ramp(["drain"], vd, sim.vd_step)

            points = self._sweep(list(GATE_CONTACTS), sim.vg_start,
                                 sim.vg_stop, sim.vg_step,
                                 vtk_tag=self._tag("idvg_vd", vd))

            vg = [p.biases["gate_top"] for p in points]
            id_a = [p.currents["drain"] * width for p in points]
            label = f"Vd = {vd:+.2f} V"
            results["curves"].append(
                {"vg": vg, "id": id_a, "vd": vd, "label": label})
            rows += [{"vg_v": g, "vd_v": vd, "id_a": i,
                      "is_a": p.currents["source"] * width}
        ...
        plot_idvg(self.out / "idvg.png", results["curves"],
                  title=f"{self.device} Id-Vg (W_eff = {width*1e4:.3f} um)")
    ...
    def run_idvd(self) -> dict:
        ...
        for vg in sim.vg:
            ...
            self._ramp(list(GATE_CONTACTS), vg, sim.vg_step)
        ...
        plot_idvd(self.out / "idvd.png", results["curves"],
                  title=f"{self.device} Id-Vd (W_eff = {width*1e4:.3f} um)")
        return results
