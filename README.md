...
- **Optimize… 对话框**（行内 Optimize 按钮 / YAML 右键）：LLM 辅助
  参数优化——见下方独立小节。
...
```bash
pip install -e '.[llm]'         # 安装 anthropic SDK
export ANTHROPIC_API_KEY=sk-...  # 未设置时 Optimize 对话框会给出明确提示
```

对已有设计右键 → **Optimize…**（或某行的 Optimize 按钮），配置优化
目标（度量 + 最大化/最小化 + 可选约束，如 "maximize ion_ioff_ratio,
constrain ss_mv_per_dec < 75"）、每轮候选数与最大轮数后开始：LLM 每
轮一次性提出 N 组几何/掺杂/功函数/迁移率标定参数候选（`src/cfet_tcad/
optimize/schema.py` 定义可调参数与合法区间），候选先经与命令行/GUI
同一套 `workflow.config.build_config` 校验（越界或非法组合在**仿真前**
被拒并要求当轮修复），合法的再复用现有多进程隔离机制并行真实求解、
把完整 FOM 结果整体喂回 LLM 做下一轮——这是**真实仿真验证**,不是
LLM 自说自话。监控窗口非模态（可以边跑边继续用主窗口），实时展示
每个候选的状态与打分，"Adopt best…" 一键把当前最优候选存成新设计
（等价于参数编辑弹窗的 Save As）。

Provider 层是可插拔适配器接口（`optimize.llm_provider.LLMProvider`），
当前内置 Claude（`anthropic` SDK，结构化 tool-call 输出，默认模型
`claude-sonnet-5`）；未装 `[llm]` 或未设置 API key 时,Optimize 设置
对话框会以清晰报错拒绝开始，不影响其余功能。这是纯 GUI 集成，暂无
命令行等价。
...
├── optimize/        # LLM 参数优化: schema.py / objective.py / llm_provider.py /
│                    #   prompt.py / orchestrator.py / llm_worker.py / claude_provider.py
...
tests/               # pytest: 几何/加载/提取/配置/求解/GUI/STEP 导入/3D 渲染/LLM 优化器
```
...
- LLM 辅助参数优化（可插拔 provider，批量候选并行验证，Claude 内置）✔
...
- 运行测试：`pytest -m "not slow"` 数十秒完成快速子集；全量 `pytest`
  （190 个上下，含 2D/3D 全流程与 STEP 导入的慢速用例）约数分钟，
  Linux CI 对每次 push 跑全量（含 `[llm]` extra，LLM 优化器用假
  provider/假 CLI 测试，不需要网络或真实 API key）。
