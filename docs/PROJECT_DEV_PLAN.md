...
| 19 | 论文 Fig4 CFET 的 .step 内置示例（STEP 导入 dogfood） | 本次提交 |
...
- **测试**：`tests/` **111 个测试**，含 5+ 组位精确交叉验证
- **示例配置**：`configs/` 18 个 YAML（2D/3D 纳米片、CFET 堆叠、SiGe、
  量子修正、VTC、论文复现 5 个）+ STEP 导入示例三件套
  （`paper_fbc_cfet_demo.step` + 映射 spec + 运行配置）
- **提交数**：52（主分支 `claude/cfet-tcad-simulation-zh2kfo`）
...
- [ ] Windows 打包需再次手动触发以纳入 STEP 导入（计划 17）与
      GUI 分辨率自适应/面板伸缩修复（计划 18）
