...
**全量归档：`docs/DEV_PLANS_ARCHIVE.md`** —— 本项目全部 **24 份**
...
| 15 | GUI 进度百分比 + 单实验停止/删除 | `92c3b99` |
| 16 | GUI 重构：单条目操作按钮 + 单窗口布局 + YAML 文件管理 | `f10bdbd` |
| 17 | STEP(.step)3D 模型导入 | `9c014ca` |
| 18 | GUI 分辨率自适应 + 面板伸缩修复 | `80a79ad` |
| 19 | 论文 Fig4 CFET 的 .step 内置示例（STEP 导入 dogfood） | `bf9ca15` |
| 20 | 双 sheet SBC CFET 的 .step 内置示例 | `9dd960d` |
...
`dev_plan_gui_single_window_rework.md`、`dev_plan_step_import.md`、
...
- **CI**：Linux 全套 pytest 自动跑（每次 push，含 `[llm]` extra）；
  Windows 打包（PyInstaller + Nuitka 两条独立跑道，均含 `[llm]`）
  **手动触发**（`workflow_dispatch` 或 `v*` 标签）
- **GUI**：PySide6 桌面工作台，单窗口三区复合布局（Experiments 表
  在上、Results 左下、Structure 3D 右下，分割条可调），每实验行内嵌
  Run/Stop/Sweep/Structure/**Optimize** 按钮，参数编辑为双击 YAML
  弹窗（Save/Save As），YAML 右键 Edit/Add/**Optimize**/Copy/Delete，
  菜单栏 Open + Help，应用图标已配
- **LLM 参数优化**（可选，`[llm]` extra）：`src/cfet_tcad/optimize/`
  —— 可插拔 `LLMProvider` 适配器接口 + 内置 Claude 实现,批量候选
  并行验证(复用 RunQueue),仿真前 dataclass 校验(轮内修复重试),
  跨轮归一化目标打分,非模态监控窗口 + Adopt Save-As
...
### Phase 9 — GUI 工作流重构（计划 14–16）
多沟道几何（`21076d9`，双 fin/双 sheet 电流精确 2× 验证）与两轮 GUI
增强：进度百分比 + 单实验停止/删除（`92c3b99`），以及单窗口重构
（`dev_plan_gui_single_window_rework.md`）——实验行内嵌
...
### Phase 10 — 全工程审查整改(计划 23)
三路并行审查(核心求解/GUI/基建,~40 条发现,报告
`docs/code_review_2026-07.md`)后落地 P0+P1:中途不收敛保存部分
CSV(`_flush_partial`)、GUI QProcess `errorOccurred`/关窗停止队列/
运行失败弹窗(带输出尾部)、配置验证补口(零步长/空偏置列表/icrit/
section 前缀/显式 dispatcher/type↔structure 建网格前校验/n_sheets
与几何复制互斥)、版本 0.5.2 动态单源、五种实验类型端到端冒烟 +
GUI live 进程路径测试(120→136)、README 大改(STEP 导入/cfet_idvd/
单窗口 GUI)。P2 打磨项(网格缓存、持久化、i18n 等)留报告备查。

### Phase 11 — LLM 器件参数优化器(计划 24)
探讨了 AgenticTCAD 论文(arXiv 2512.23742,LLM 多智能体驱动 Sentaurus
代码生成+器件优化)后决定演进方向:本工程已有类型化校验的 YAML 配置层
天然绕开该论文最难的"NL→TCAD 脚本生成"问题,LLM 只需吐结构化参数
字典。新增 `src/cfet_tcad/optimize/` 包(纯 Python 核心:
`schema.py` 手工维护可调参数边界表、`objective.py` 跨轮归一化打分、
`llm_provider.py` 可插拔 provider 抽象 + `FakeProvider`、
`prompt.py` 结构化 prompt 构建;GUI 依赖层:`orchestrator.py`
QThread 驱动的轮次循环控制器 + 复用 RunQueue 并行验证、
`llm_worker.py` 跨线程调用;`claude_provider.py` 懒加载 anthropic
的 Claude 实现)。设计要点:批量候选并行验证(每轮 N 个候选走同一套
仿真前 dataclass 校验 + RunQueue 并行,越界候选轮内修复重试而非
浪费仿真);目标打分维护跨轮归一化范围(修复了"新高点总是自归一化
为 1.0 导致 best-so-far 追踪错误"的算法 bug);`gui/optimize_dialog.py`
新增 `OptimizeSetupDialog`(目标/候选数/轮数/provider)+ 非模态
`OptimizeMonitorDialog`(实时结果表 + Adopt best Save-As)+
`OptimizeExperimentModel`(免费复用主表列 + 新增 Score 列);
`MainWindow` 新增行内 Optimize 按钮/右键项/`closeEvent` 扩展。
新增 `pyproject.toml` `[llm]` extra(独立于 `[gui]`,已加入 CI 与两条
...
- [x] 真实 2-fin/2-sheet 几何的论文 Fig4/8/9 重渲染已交付（图脚本
      cutaway 改为按连通体裁剪，`_clip_open`）
- [x] Windows 打包已在 `f10bdbd` 重新构建全绿（2026-07-06 手动触发）：
      含多沟道几何、GUI 进度/停止/删除、GUI 单窗口重构
- [ ] Windows 打包需手动触发以纳入 STEP 修复三连（计划 19/20 后的
      convert/run 修复）、`cfet_idvd`、手册/截图刷新与本次审查整改
      （P0/P1 的 GUI 健壮性与配置验证都在 exe 里生效）
