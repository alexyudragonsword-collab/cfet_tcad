...
最后更新：全工程审查整改 P0+P1（分支 `claude/cfet-tcad-simulation-zh2kfo`，
完整审查报告见 `docs/code_review_2026-07.md`）。
...
| 23 | 全工程审查整改:P0 关键缺陷 + P1 重要改进 | 本次提交 |
...
- **源码**：`src/cfet_tcad/` 约 6710 行（49 个模块文件）
- **测试**：`tests/` **136 个测试**，含 5+ 组位精确交叉验证与全部
  五种实验类型的端到端冒烟
- **示例配置**：`configs/` 21 个 YAML（2D/3D 纳米片、CFET 堆叠
  转移/输出/VTC、SiGe、量子修正、论文复现 5 个）+ STEP 导入示例
  两套六件（FBC/SBC 各 `.step` + 映射 spec + 运行配置）
- **提交数**：64（主分支 `claude/cfet-tcad-simulation-zh2kfo`）
...
- **GUI**：PySide6 桌面工作台，单窗口三区复合布局（Experiments 表
  ...
  Run/Stop/Sweep/Structure 按钮，参数编辑为双击 YAML 弹窗
  （Save/Save As），YAML 右键 Edit/Add/Copy/Delete，菜单栏
  Open + Help，应用图标已配
